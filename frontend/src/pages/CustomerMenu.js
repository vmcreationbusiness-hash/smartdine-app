      ...
      <header className="glass-header sticky top-0 z-20">
        <div className="max-w-[1400px] mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 md:gap-3">
            {loyaltyPoints > 0 && (
              <div className="flex items-center gap-1 px-3 py-1.5 rounded-full text-white text-xs font-semibold" style={{ background: 'linear-gradient(135deg, ' + theme.secondaryColor + ', ' + theme.primaryColor + ')' }}>
                <Star className="w-3 h-3 fill-white" />
                <span>{loyaltyPoints} {t('menu.loyalty_points')}</span>
              </div>
            )}
            <Button onClick={() => navigate('/customer/orders')} variant="ghost" size="sm" className="text-xs font-semibold">
        ...
        <div className="flex-1 p-4 md:p-6 overflow-y-auto">
          ...
          {categories.length > 1 && (
            <div className="flex gap-2 mb-5 overflow-x-auto pb-1">
              {categories.map((cat) => (
                <button key={cat} onClick={() => setActiveCategory(cat)} className={'category-pill flex-shrink-0 ' + (activeCategory === cat ? 'active' : 'bg-white/50 text-gray-600')} style={activeCategory === cat ? { background: 'linear-gradient(135deg, ' + theme.primaryColor + ', ' + theme.secondaryColor + ')', borderColor: 'transparent' } : { borderColor: 'rgba(0,0,0,0.1)' }}>
                  {cat}
                </button>
              ))}
            </div>
